...
static ssize_t adc_max1193_write(struct file *filp, const char __user *buff, size_t count, loff_t *f_pos) {
	ssize_t status;
	size_t len = USER_BUFF_SIZE - 1;
	if (count == 0) return 0;
	if (down_interruptible(&adc_max1193_dev.sem)) return -ERESTARTSYS;
	if (len > count) len = count; memset(adc_max1193_dev.user_buff, 0, USER_BUFF_SIZE);
	...
	status = len;
	*f_pos += len;
	adc_max1193_write_done: up(&adc_max1193_dev.sem);
	return status;
}
static ssize_t adc_max1193_read(struct file *filp, char __user *buff, size_t count, loff_t *offp) {
	ssize_t status; size_t len; /* 	Generic user progs like cat will continue calling until we 	return zero. So if *offp != 0, we know this is at least the	second call.	*/
	if (*offp > 0) return 0;
	if (down_interruptible(&adc_max1193_dev.sem))
	return -ERESTARTSYS; strcpy(adc_max1193_dev.user_buff, "adc_max1193 driver data\n");
	...
	*offp += len;
	status = len;
	adc_max1193_read_done: up(&adc_max1193_dev.sem);
	return status;
}
...
static const struct file_operations
		adc_max1193_fops = { .owner = THIS_MODULE, .open = adc_max1193_open,
				.read = adc_max1193_read, .write = adc_max1193_write, };
static int __init adc_max1193_init_cdev(void) {
	int error; adc_max1193_dev.devt = MKDEV(0, 0);
	error = alloc_chrdev_region(&adc_max1193_dev.devt, 0, 1, "adc_max1193");
	if (error < 0) {printk(KERN_ALERT "alloc_chrdev_region() failed: error = %d \n", error);
		return -1;
	}
...
static int __init adc_max1193_init(void) {
	...
	if (adc_max1193_init_cdev()) goto init_fail_1;
	if (adc_max1193_init_class()) goto init_fail_2;
	return 0;
	init_fail_2: cdev_del(&adc_max1193_dev.cdev);
	unregister_chrdev_region(adc_max1193_dev.devt, 1);
	init_fail_1: return -1;
}
module_init( adc_max1193_init);
static void __exit adc_max1193_exit(void) {printk(KERN_INFO "adc_max1193_exit()\n"); device_destroy(adc_max1193_dev.class, adc_max1193_dev.devt); class_destroy(adc_max1193_dev.class); cdev_del(&adc_max1193_dev.cdev); unregister_chrdev_region(adc_max1193_dev.devt, 1); if (adc_max1193_dev.user_buff) kfree(adc_max1193_dev.user_buff);}
